...
| 4 | Designer diagnostics: pool stats, execution timing, gateway impact visible **without gateway web access** | Designer | ⬜ re-test on v4.4.0 — **defects found & fixed 06/07/2026** (maintainer + audit): Total Executions / Success Rate / Avg Time were structurally always 0, Impact/Health frozen at LOW/100, Py3 CPU% a fixed guess. All rewired to live pool metrics in v4.4.0 (unit-tested); needs a visual confirm that the numbers now move after running scripts |
...
| 10 | Version identical in Designer, web UI, gateway module list; uninstall → zero orphaned Python processes | All | ⬜ uninstall ✅ 04/07/2026 (module removed → routes 404, **zero** python processes, clean logs); version-identity to re-confirm on v4.3.6 (was ✅ on v4.2.0) |
...
- [ ] **Two acceptance boxes remain (maintainer visual confirm on v4.5.2):** workflow 4 (diagnostics numbers move after running scripts — rewired v4.4.0) and workflow 10 (version identity reads 4.5.2 in Designer, web UI, and gateway module list). These are the only boxes left before the v4.3.0→v4.5.2 release is fully acceptance-clean.
